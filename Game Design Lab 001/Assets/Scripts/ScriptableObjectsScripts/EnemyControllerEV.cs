...
public class EnemyControllerEV : MonoBehaviour
{
	...
	private AudioSource enemyStompSFX;
	...
	void Start()
	{
		enemyStompSFX = GetComponent<AudioSource>();
	...
	void changeSprite() //function to flip enemy sprite
	{
		if (moveRight == 1)
		{
			GetComponent<Animator>().SetBool("faceRight", true); //set the parameter in the Animator for faceRight to true
		}
		else
		{
			GetComponent<Animator>().SetBool("faceRight", false); //faceRight is now false
		}
	}
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		if (other.gameObject.tag == "Player")
		{
			...
			if (yoffset > 0.75f && enemyAlive)
			{
				...
				onEnemyDeath.Invoke(); //invoke event onEnemyDeath
			}
			else if (yoffset < 0.75f && enemyAlive)
			{
				// hurt player
				onPlayerDeath.Invoke(); //invoke onPlayerDeath
			}
		}
	}
	...
	// callback for Response must be PUBLIC
	public void PlayerDeathResponse() //signature must match no arguments and returns void
	{
		marioAlive = false; //mario is now dead, enemy stop moving
		GetComponent<Animator>().SetBool("playerIsDead", true); //set the parameter in the Animator for playerIsDead to true
		velocity = Vector3.zero;
	}

	public void PlayAudioStomped()
    {
		enemyStompSFX.PlayOneShot(enemyStompSFX.clip);
	}
}